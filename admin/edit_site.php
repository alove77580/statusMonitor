...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE monitor_sites SET name = ?, url = ?, type = ?, check_interval = ?, is_active = ? WHERE id = ?");
            
            if ($stmt->execute([$name, $url, $type, $checkInterval, $isActive, $id])) {
                $success = '监控网站更新成功';
                // 更新本地数据
                $site['name'] = $name;
                $site['url'] = $url;
                $site['type'] = $type;
                $site['check_interval'] = $checkInterval;
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">
                            <i class="fas fa-edit me-2"></i>编辑监控网站
                        </h4>
                    </div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <form method="POST">
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="url" class="form-control" id="url" name="url" value="<?php echo htmlspecialchars($site['url']); ?>" required placeholder="https://www.example.com">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" id="type" name="type">
                                    <option value="HTTP" <?php echo $site['type'] == 'HTTP' ? 'selected' : ''; ?>>HTTP</option>
                                    <option value="HTTPS" <?php echo $site['type'] == 'HTTPS' ? 'selected' : ''; ?>>HTTPS</option>
                                    <option value="API" <?php echo $site['type'] == 'API' ? 'selected' : ''; ?>>API</option>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="d-flex align-items-center">
                                    <input type="range" class="form-range flex-grow-1 me-3" id="check_interval" name="check_interval" 
                                           min="15" max="3600" step="1" value="<?php echo $site['check_interval']; ?>" 
                                           oninput="updateIntervalDisplay(this.value)">
                                    <span class="badge bg-primary fs-6" id="interval_display"><?php echo formatInterval($site['check_interval']); ?></span>
                                </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" <?php echo $site['is_active'] ? 'checked' : ''; ?>>
